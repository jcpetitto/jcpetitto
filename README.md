...
Currently, I am working on an [pipeline for processing and analyzing single-molecule tracking data]{.underline} applying [neural networks]{.underline} and [likelihood estimation]{.underline} to segmentation and particle tracking, which we are using to study mRNA transport through the nuclear pore complex in yeast.
...
My work has been done locally, on both Windows and Mac systems, as well as on [high-performance computing clusters]{.underline} using *command-line* tools.

My graduate-level coursework included applying [machine-learning]{.underline} techniques to LIDAR and fNIRs datasets as well as [modeling protein-ligand interactions]{.underline} with tools such as *Chimera* and *Rosetta*.
